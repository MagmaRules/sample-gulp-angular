...
searchApp.controller('SearchCtrl', ['$scope', '$http','$timeout', function ($scope, $http, $timeout) {
    ...
    function processResults(results){
        ...
        for(var i in results){
            ...
            entry.endOfAnEra = zeroPad(date.getDate(),2) + "/" + zeroPad(date.getMonth() + 1, 2) + "/" + zeroPad(date.getFullYear().toString(), 4);
    ...
    $scope.showError = function(){
        showLoading();
        $http.get('https://athena-7.herokuapp.com/ancients.json?error=true').error(function(data) {
    ...
    $scope.search = function(query){
        ...
        showLoading();
        $http.get('https://athena-7.herokuapp.com/ancients.json?search=' + query).success(function(data) {
            loadResults(data);
        });
        lastQuery = query;
    };
